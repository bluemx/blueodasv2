...
app.component('util-text', {
    ...
    setup(props, context){
        ...
        const itemColor = ref(props.color || randomcolor.toString())
        const itemSize = ref(props.size || 'lg')


        
        //-Colors
        switch(itemColor.value){
            case "1":
                itemClass.value += ' text-main'
                break;
            case "2":
                itemClass.value += ' text-sec'
                break;
            case "3":
                itemClass.value += ' text-clear'
                break;
            case "4":
                itemClass.value += ' text-accent'
                break;
        }

        itemClass.value += ' text-'+itemSize.value
        ...
        if(itemDesign.value == 2){
            itemClass.value += '  font-extrabold relative before:content-[attr(data)] [text-shadow:0px_2px_0px_rgba(255,255,255,.8)]  before:absolute before:left-0 before:top-1  before:z-0 before:opacity-50 after:content-[attr(data)] after:absolute after:left-0 after:top-1.5  after:z-0 after:opacity-30  before:-z-10 after:-z-10  '
        }
        ...
        if(props.bold!=undefined){
            itemClass.value +=' font-bold'
        }
        if(props.extrabold!=undefined){
            itemClass.value +=' font-extrabold'
        }
